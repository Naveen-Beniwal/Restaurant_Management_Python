...
def leave_table():
    ...
    if(i>1):
      i=i-1
      showtable.config(text="Table no. " + str(i) + " is Unbooked", bg="lightgreen", fg="black")
      enable_order()
      clear_entries()
...
def checktable():
    global i, showtable
    if i <= 20:
        showtable.config(text="Table no. " + str(i) + " is Booked", bg="lightgreen", fg="black")
        enable_order() 
        clear_entries()  
        i += 1
    else:
        showtable.config(text="All tables are already booked!", bg="red", fg="white")
        disable_order() 
...
tablecheck = tk.Button(root, text="Book Table", font=("arial", 16), command=checktable)
